...
function fillInfo(plantChoice) {
  // fills the info table with the selected plant's info
...
function calculateEC(currentEC, currentPlant) {
  // calculates the amount of cups of NPK mixture to add to the water
  EcResult.innerHTML = "";
  for (let i = 0; i < plantInfo.length; i++) {
    if (currentPlant == plantInfo[i].name) {
      ...
      if (currentEC < currentPlant.ec) {
        ...
        EcResult.innerHTML = "Your E.C. is already high enough";
      } else if (currentEC > currentPlant.ec) {
        EcResult.innerHTML = "Your E.C. is too high, add water to mixture.";
      }
    }
  }
}
